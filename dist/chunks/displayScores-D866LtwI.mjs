...
async function fetchScoresTxts() {
	...
	try {
		...
		if (statusEl) statusEl.textContent = `Fetching ${selectedIds.length} PGS file(s) for GRCh${build}...`;
		...
		const pgsTxts = (await Promise.all(selectedIds.map(id => getPgsTxt(id, undefined, true, build)))).flat();
...
//# sourceMappingURL=displayScores-D866LtwI.mjs.map
