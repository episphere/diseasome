...
// Same for the genome-cache tools, which sit in the table's button row.
let _genomicCacheControls = null;
let _genomicCacheList = null;
...
// Update the textual genomic-data loading status (mirrors the PGS status on tab 2).
const genomicLoadingStatusEl = document.getElementById('genomicLoadingStatus');
function setGenomicLoadingStatus(message, isError = false) {
	if (!genomicLoadingStatusEl) return;
...
try {
	...
	setGenomicLoadingStatus(`Failed to load genomic data: ${err.message}`, true);
}
setParticipantsLoadingProgress(100);

let participants = curatedJsonParticipants ?? [];
if (participants.length) {
	setGenomicLoadingStatus(`Genomic data loaded successfully. ${participants.length} participants from curated list.`);
}
...
window.clearPGPSelections = function () {
	...
	window.removeLoadedUsers?.(removed);
...
function updateGlobalSelectionCount() {
	...
	if (fetchBtn) {
		...
		// Reset to red + default label whenever selection changes (new upload or PGP
		// selection). Skip while a load is in flight so the spinner is not clobbered.
		if (!fetchBtn.disabled) {
			fetchBtn.classList.remove('btn-secondary', 'btn-success');
			fetchBtn.classList.add('btn-danger');
			fetchBtn.innerHTML = 'Load File(s)';
			const fetchWrap = document.getElementById('fetchUsersProgressWrap');
...
/**
 * Render the current selection into #prsUsersAction.
 * Delegates to the single render path in calculatePrs.js (window.renderPrsUsersTable),
 * which merges this selection (uploads / PGP participants) with users loaded there
 * (examples / fetched) so neither source overwrites the other.
 */
function renderSelectedUsersTable() {
	if (typeof window.renderPrsUsersTable === "function") window.renderPrsUsersTable();
}
/**
 * escapeHtml(value)
...
function renderParticipantsTable(list, targetId, title, key) {
	...
	const renderPage = () => {
		...
		const sortAttrsEnd = (k) => `class="sortable" data-sort="${k}" style="cursor:pointer;user-select:none;"`;
		...
		container.innerHTML = `
			...
			<div class="d-flex justify-content-between align-items-center gap-2 flex-wrap mb-2">
				...
				<div class="d-flex align-items-center gap-2 flex-wrap">
					...
					<span id="genomicCacheSlot_${key}" class="d-inline-flex align-items-center gap-2"></span>
				</div>
				<div id="genomicCacheListSlot_${key}" class="w-100"></div>
			</div>
		...
		// Move the genome-cache tools into the table's button row, with their output
		// (the cached-file list) on the line directly below it.
		const cacheSlot = document.getElementById(`genomicCacheSlot_${key}`);
		if (!_genomicCacheControls) _genomicCacheControls = document.getElementById('genomicCacheControls');
		if (cacheSlot && _genomicCacheControls) cacheSlot.appendChild(_genomicCacheControls);
		const cacheListSlot = document.getElementById(`genomicCacheListSlot_${key}`);
		if (!_genomicCacheList) _genomicCacheList = document.getElementById('genomicCacheList');
		if (cacheListSlot && _genomicCacheList) cacheListSlot.appendChild(_genomicCacheList);
		if (_genomicCacheControls && _genomicCacheList) {
			const emptied = document.getElementById('genomicCacheBlock');
		...
		if (firstPageBtn) firstPageBtn.addEventListener('click', () => { currentPage = 1; renderPage(); });
		...
		if (lastPageBtn) lastPageBtn.addEventListener('click', () => { currentPage = totalPages; renderPage(); });
...
// The v4/v5 23andMe SNP overlap (window.v4_23andme / v5_23andme / v4_v5_23andme) is
// computed in displayScores.js instead, so the two reference genomes are only fetched
// when the Select Risk Models tab is opened rather than on every Genomic Data visit.
