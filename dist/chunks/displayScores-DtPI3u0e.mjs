...
// Same for the PGS cache tools, which sit in the table's button row.
let _pgsCacheControls = null;
let _pgsCacheInfo = null;
...
function updateGlobalSelectionCount() {
	...
	if (fetchPgsFilesBtn) {
		...
		// Reset to the default label/colour whenever the selection changes. Skip while a
		// load is in flight so the spinner state is not clobbered (mirrors tab 1).
		if (!fetchPgsFilesBtn.disabled) {
			fetchPgsFilesBtn.classList.remove("btn-secondary", "btn-danger");
			fetchPgsFilesBtn.classList.add("btn-success");
			fetchPgsFilesBtn.innerHTML = "Fetch PGS Files";
			const fetchWrap = document.getElementById("fetchScoresProgressWrap");
			if (fetchWrap) fetchWrap.style.display = "none";
		}
	}
...
function renderPgsFromSelection() {
	...
	const title = `Model Scoring Files - ${scores.length} of ${totalAvailableScores}`;
...
/** Explanations shown by the "?" badge on each Select Risk Models table column. */
const PGS_COL_HELP = {
	select: "Select this risk model for PRS calculation. Selected models are loaded into the Calculate PRS tab.",
	...
	weightType: "How the effect weights were derived, as reported to the PGS Catalog (e.g. beta, log(OR), log(HR)). NR = not reported to the catalog — the weight type may still be found in the score's original publication. Never inferred from the weights themselves.",
	variants: "Total number of variants in the model according to the PGS Catalog metadata.",
	v4: "Fraction of this model's variants covered by the 23andMe v4 chip — an upper bound on the match rate for v4 genomes.",
	v5: "Fraction of this model's variants covered by the 23andMe v5 chip — an upper bound on the match rate for v5 genomes.",
...
function renderPgsTable(scores, targetId, title, key) {
	...
	let currentPage = 1;
	let searchQuery = "";
	let sortState = { key: null, dir: "asc" }; // key: id|name|trait|weighttype|variants|date
	...
	const renderPage = () => {
		...
		const sortGetters = {
			...
			weighttype: (s) => String(s?.weight_type ?? "NR").toLowerCase(),
			variants: (s) => {
		...
		const rowsHtml = pageScores
			.map((score, index) => {
				...
				// Reported by the PGS Catalog (`weight_type`); "NR" = not reported. Never inferred.
				const weightType = escapeHtml(score?.weight_type ?? "NR");
		...
		scoresDiv.innerHTML = `
			...
			<div class="d-flex justify-content-between align-items-center gap-2 flex-wrap mb-2">
				...
				<div class="d-flex align-items-center gap-2 flex-wrap">
					...
					<span id="pgsCacheSlot_${key}" class="d-inline-flex align-items-center gap-2"></span>
				</div>
				<div id="pgsCacheInfoSlot_${key}" class="w-100"></div>
			</div>
			<div class="table-responsive sticky-scroll">
				<table class="table table-sm table-striped table-bordered align-middle">
					<thead  class="table-dark">
						<tr>
							<th>#</th>
							<th>Select${colHelp(PGS_COL_HELP.select)}</th>
							<th ${sortAttrs('id')}>PGS ID${colHelp(PGS_COL_HELP.id)}${sortArrow('id')}</th>
							<th ${sortAttrs('name')}>Name${colHelp(PGS_COL_HELP.name)}${sortArrow('name')}</th>
							<th ${sortAttrs('trait')}>Trait${colHelp(PGS_COL_HELP.trait)}${sortArrow('trait')}</th>
							<th ${sortAttrs('weighttype')}>Weight Type${colHelp(PGS_COL_HELP.weightType)}${sortArrow('weighttype')}</th>
							<th ${sortAttrs('variants')}>Variants #${colHelp(PGS_COL_HELP.variants)}${sortArrow('variants')}</th>
							<th ${sortAttrs('v4overlap')}>v4 chip %${colHelp(PGS_COL_HELP.v4)}${sortArrow('v4overlap')}</th>
							<th ${sortAttrs('v5overlap')}>v5 chip %${colHelp(PGS_COL_HELP.v5)}${sortArrow('v5overlap')}</th>
							<th ${sortAttrs('date')}>Date${colHelp(PGS_COL_HELP.date)}${sortArrow('date')}</th>
						</tr>
					</thead>
					<tbody>
						${rowsHtml}
			...
			<div class="d-flex justify-content-end align-items-center mt-2">
				<div class="d-flex align-items-center gap-2">
					<button id="firstPage_${key}" class="btn btn-sm btn-outline-secondary" ${currentPage === 1 ? "disabled" : ""}>&laquo; First</button>
					<button id="prevPage_${key}" class="btn btn-sm btn-outline-secondary" ${currentPage === 1 ? "disabled" : ""}>&lsaquo; Previous</button>
					<span id="pageInfo_${key}" class="small text-muted">Page ${currentPage} of ${totalPages}</span>
					<button id="nextPage_${key}" class="btn btn-sm btn-outline-secondary" ${currentPage >= totalPages ? "disabled" : ""}>Next &rsaquo;</button>
					<button id="lastPage_${key}" class="btn btn-sm btn-outline-secondary" ${currentPage >= totalPages ? "disabled" : ""}>Last &raquo;</button>
				</div>
			</div>
		`;
		...
		// Move the PGS cache tools into the table's button row, with their output
		// (the cache summary line) on the line directly below it.
		const pgsCacheSlot = document.getElementById(`pgsCacheSlot_${key}`);
		if (!_pgsCacheControls) _pgsCacheControls = document.getElementById('pgsCacheControls');
		if (pgsCacheSlot && _pgsCacheControls) pgsCacheSlot.appendChild(_pgsCacheControls);
		const pgsCacheInfoSlot = document.getElementById(`pgsCacheInfoSlot_${key}`);
		if (!_pgsCacheInfo) _pgsCacheInfo = document.getElementById('pgsCacheInfo');
		if (pgsCacheInfoSlot && _pgsCacheInfo) pgsCacheInfoSlot.appendChild(_pgsCacheInfo);
		if (_pgsCacheControls && _pgsCacheInfo) {
			const emptied = document.getElementById('pgsCacheBlock');
			if (emptied) emptied.style.display = 'none';
		}
		...
		const firstPageBtn = document.getElementById(`firstPage_${key}`);
		const prevPageBtn = document.getElementById(`prevPage_${key}`);
		const nextPageBtn = document.getElementById(`nextPage_${key}`);
		const lastPageBtn = document.getElementById(`lastPage_${key}`);
...
function scoresToCsv(list) {
	const cols = ["id", "name", "trait_reported", "weight_type", "variants_number", "date_release"];
...
async function fetchScoresTxts() {
	...
	// Tab-2 "Fetch PGS Files" button + inline loading bar feedback (mirrors tab 1).
	const btn = document.getElementById("fetchPgsFilesBtn");
	const btnOriginalHtml = btn ? btn.innerHTML : null;
	const progressWrap = document.getElementById("fetchScoresProgressWrap");
	const progressBar = document.getElementById("fetchScoresProgressBar");
	...
	// Loading state: spinner + disabled, reveal the bar.
	if (btn) {
		btn.disabled = true;
	...
	try {
		...
		if (selectedIds.length === 0) {
			setProgress(0);
			setStatus("Please select at least one scoring file from the table above before loading.", "error");
			if (btn) {
				btn.classList.remove("btn-secondary", "btn-danger");
				btn.classList.add("btn-success");
				btn.innerHTML = btnOriginalHtml ?? "Fetch PGS Files";
			}
			return;
		}

		const build = getPgsBuild();
		setStatus(`Loading 0 of ${selectedIds.length} model(s) for GRCh${build}…`);
		setProgress(5);
		if (txtsDiv) txtsDiv.style.display = "block";

		// Fetch PGS text files using the SDK (getPgsTxt loads one id at a time,
		// returning an array each). Track completions + per-id success as promises
		// settle so the bar advances and failures can be reported as comments.
		// The 4th arg is the genome build (37/38); GRCh37 and GRCh38 cache separately.
		let done = 0;
		let succeeded = 0;
		const total = selectedIds.length;
		const results = await Promise.all(selectedIds.map(id =>
			getPgsTxt(id, undefined, true, build)
				.then(r => { const arr = Array.isArray(r) ? r : (r ? [r] : []); if (arr.length) succeeded++; return arr; })
				.catch(err => { console.error(`Failed to load PGS ${id}:`, err); return []; })
				.finally(() => {
					done++;
					setProgress(5 + (done / total) * 90);
					setStatus(`Loading ${done} of ${total} model(s)…`);
				})
		));
		const pgsTxts = results.flat();
		...
		// Completion feedback, with error comments for any models that failed.
		setProgress(100);
		...
		if (failedCount > 0) {
			...
			setStatus(`Loaded ${succeeded} of ${total} model(s). <span class="fw-semibold">${failedCount} model(s) failed to load</span> — the PGS Catalog file server may be unreachable. See the browser console for details.`, "error");
			if (btn) {
			...
			setStatus(`Loaded ${succeeded} of ${total} model(s).`, "success");
			if (btn) {
				btn.classList.remove("btn-danger", "btn-success");
				btn.classList.add("btn-secondary");
		...
		<div class="table-responsive">
			<table class="table table-sm table-striped">
				<thead class="table-dark">
					<tr>
						<th>#</th>
						<th>PGS ID${colHelp(PGS_COL_HELP.id)}</th>
						<th>Trait${colHelp(PGS_COL_HELP.trait)}</th>
						<th>Variants${colHelp("Number of variant rows parsed from the harmonized (GRCh37) scoring file into memory.")}</th>
					</tr>
				</thead>
				<tbody>${rows}</tbody>
			</table>
...
// Genome build (37 or 38) selected in the Select Risk Models tab. Determines which
// harmonized scoring file (GRCh37 vs GRCh38) getPgsTxt() fetches; the SDK caches the
// two builds under separate keys. Defaults to 37 when the selector is absent.
function getPgsBuild() {
	const checked = document.querySelector('input[name="pgsBuild"]:checked');
	return Number(checked?.value) === 38 ? 38 : 37;
}
window.getPgsBuild = getPgsBuild;
...
//# sourceMappingURL=displayScores-DtPI3u0e.mjs.map
