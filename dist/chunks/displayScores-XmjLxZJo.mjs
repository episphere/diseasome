...
// --- 23andMe v4/v5 reference SNP sets ---------------------------------------
// Two reference genomes are parsed to derive the chip SNP sets that
// compareSnpOverlap() scores each PGS file against. This lives here (rather than in
// displayUsers.js) so the files are only fetched once the Select Risk Models tab is
// opened — that tab is the sole consumer, and the Genomic Data tab no longer pays for it.
...
/**
 * Compute overlapping SNP positions between a v5 and a v4 23andMe file, and publish
 * the resulting sets on window for compareSnpOverlap().
 * @returns {Promise<string[]>} Overlapping "chr:position" keys
 */
...
// Kick off on module load. tabs.js imports this module when the Select Risk Models tab
// is first opened, so the work starts then; the promise is stored so fetchScoresTxts()
// can await it rather than racing the parse.
window._v4v5OverlapReady = computeV4V5Overlap();
window.computeV4V5Overlap = computeV4V5Overlap;
...
//# sourceMappingURL=displayScores-XmjLxZJo.mjs.map
